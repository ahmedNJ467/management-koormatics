...
export function useCostAnalyticsData(selectedYear: string) {
      ...
      try {
        ...
        const partsWithRelationships = relevantParts.map((partData: any) => {
          ...
          const part: SparePart = {
            ...partData,
            status: partData.status as string as
              | "in_stock"
              | "low_stock"
              | "out_of_stock",
            vehicles: null,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (maintenanceError) {
          console.error(
            "Maintenance records fetch error for comparison:",
            maintenanceError
          );
          return [];
        }
  ...
  useEffect(() => {
    ...
    if (sparePartsError)
      console.error("Spare parts query error:", sparePartsError);
    if (comparisonMaintenanceError)
      console.error(
        "Comparison maintenance query error:",
        comparisonMaintenanceError
      );
    if (comparisonFuelError)
      console.error("Comparison fuel query error:", comparisonFuelError);
    if (comparisonTripsError)
      console.error("Comparison trips query error:", comparisonTripsError);
    if (comparisonSparePartsError)
      console.error(
        "Comparison spare parts query error:",
        comparisonSparePartsError
      );
  }, [
    maintenanceError,
    fuelError,
  ...
  const isLoading =
    ...
    sparePartsLoading ||
    (!!comparisonYear &&
      (comparisonMaintenanceLoading ||
        comparisonFuelLoading ||
        comparisonTripsLoading ||
        comparisonSparePartsLoading));
