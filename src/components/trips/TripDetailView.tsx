...
export function TripDetailView({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "scheduled":
        return "bg-blue-500/10 text-blue-600 border-blue-500/20 hover:bg-blue-500/20 dark:bg-blue-500/20 dark:text-blue-400 dark:border-blue-500/40";
      case "in_progress":
        return "bg-amber-500/10 text-amber-600 border-amber-500/20 hover:bg-amber-500/20 dark:bg-amber-500/20 dark:text-amber-400 dark:border-amber-500/40";
      case "completed":
        return "bg-green-500/10 text-green-600 border-green-500/20 hover:bg-green-500/20 dark:bg-green-500/20 dark:text-green-400 dark:border-green-500/40";
      case "cancelled":
        return "bg-red-500/10 text-red-600 border-red-500/20 hover:bg-red-500/20 dark:bg-red-500/20 dark:text-red-400 dark:border-red-500/40";
      default:
    ...
    <div className="w-full">
      <div className="bg-gradient-to-r from-muted/50 to-muted/30 dark:from-muted/70 dark:to-muted/50 p-6 rounded-lg mb-6 border border-border">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <Badge
            variant="outline"
            className={`font-medium ${getStatusColor(viewTrip.status)}`}
          >
          ...
          {hasPassengers && (
            ...
            >
              {viewTrip.passengers?.length}{" "}
              {viewTrip.passengers?.length === 1 ? "Passenger" : "Passengers"}
            </Badge>
          )}
          {viewTrip.has_security_escort && (
            <Badge
              variant="destructive"
