...
interface DispatchTripsProps {
  ...
  // Removed Assign Escort per request
  vehicles?: Vehicle[];
}
              ...
              <AccordionContent className="px-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 py-3">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Passengers - Show for airport trips or trips with passengers */}
                    {(trip.type === "airport_pickup" || 
                      trip.type === "airport_dropoff" || 
                      (trip.passengers && trip.passengers.length > 0) ||
                      (trip.passport_documents && trip.passport_documents.length > 0) ||
                      (trip.invitation_documents && trip.invitation_documents.length > 0)) && (
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                        ...
                        {trip.passport_documents && trip.passport_documents.length > 0 && (
                          <div className="mb-3">
                            ...
                            <div className="space-y-1">
                              {trip.passport_documents.map((doc, idx) => (
                                <div
                                  key={idx}
                                ...
                                >
                                  ...
                                  >
                                    {doc.name || doc.passenger_name || `Passport ${idx + 1}`}
                                  </a>
                                </div>
                              ))}
                            </div>
                        ...
                        {trip.invitation_documents && trip.invitation_documents.length > 0 && (
                          <div>
                            ...
                            <div className="space-y-1">
                              {trip.invitation_documents.map((doc, idx) => (
                                <div
                                  key={idx}
                                  className="flex items-center gap-2 text-xs"
                                >
                                  ...
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 underline truncate"
                                  >
                                    {doc.name || doc.passenger_name || `Invitation ${idx + 1}`}
                                  </a>
                                </div>
                              ))}
                            </div>
                        ...
                        {/* Show message if no passengers but documents exist */}
                        {(!trip.passengers || trip.passengers.length === 0) &&
                          ((trip.passport_documents && trip.passport_documents.length > 0) ||
                            (trip.invitation_documents && trip.invitation_documents.length > 0)) && (
                            <div className="text-xs text-muted-foreground italic">
                              No passenger names listed
                            </div>
                          )}
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {/* Assign Escort removed per request */}
