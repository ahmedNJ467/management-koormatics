...
export const fuelLogSchema = z
  .object({
    ...
    tank_id: z.union([z.string().uuid(), z.literal("")]).optional(),
  })
  ...
  .refine(
    (data) =>
      // Ensure mileage matches the computed difference (defensive; UI already sets this)
      data.mileage ===
      Math.max(0, data.current_mileage - data.previous_mileage),
    {
      message: "Mileage must equal current mileage minus previous mileage",
      path: ["mileage"],
    }
  );
