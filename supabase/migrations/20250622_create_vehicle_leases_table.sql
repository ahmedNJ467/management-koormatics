...
CREATE INDEX IF NOT EXISTS idx_vehicle_leases_assigned_driver ON vehicle_leases(assigned_driver_id);
...
COMMENT ON COLUMN vehicle_leases.driver_included IS 'Whether a driver is provided with the lease';
COMMENT ON COLUMN vehicle_leases.fuel_included IS 'Whether fuel costs are included in the lease';
COMMENT ON COLUMN vehicle_leases.assigned_driver_id IS 'Driver assigned to this lease (if driver_included is true)';
-- Create enum types for vehicle leases
CREATE TYPE lease_status AS ENUM ('active', 'pending', 'expired', 'terminated', 'upcoming');
CREATE TYPE payment_status AS ENUM ('current', 'overdue', 'partial', 'paid_ahead');
